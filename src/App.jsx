...
import { buildMiningQuery, decorateActivity, MINING_MODES, miningScore } from './lib/mining'

const PRESETS = [
  { label: 'Radar amplo BR', query: 'date:>now-7d AND page.country:BR AND (domain:api.whatsapp.com OR domain:wa.me OR domain:kiwify.com.br OR domain:pay.hotmart.com OR domain:kirvano.com OR domain:pandavideo.com OR domain:vturb.com.br)' },
  { label: 'WhatsApp Direct', query: 'date:>now-7d AND page.country:BR AND (domain:api.whatsapp.com OR domain:wa.me OR domain:whatsapp.com)' },
  { label: 'Checkout Brasil', query: 'date:>now-7d AND page.country:BR AND (domain:kiwify.com.br OR domain:pay.hotmart.com OR domain:kirvano.com OR domain:asaas.com OR domain:mercadopago.com.br)' },
  { label: 'VSL / Vídeo', query: 'date:>now-7d AND page.country:BR AND (domain:pandavideo.com OR domain:vturb.com.br OR domain:youtube.com OR domain:vimeo.com)' },
]
...
export default function App() {
  ...
  function mineNow() {
    const q = buildMiningQuery({ days, country: 'BR', mode, term })
    setQuery(q)
    runSearch(q)
  }
  ...
  return <div className="min-h-screen bg-[radial-gradient(circle_at_top_left,rgba(16,185,129,.08),transparent_28%),radial-gradient(circle_at_top_right,rgba(59,130,246,.07),transparent_28%)] bg-zinc-950 text-zinc-100">
    ...
    <main className="mx-auto max-w-[1700px] px-4 py-6 lg:px-8">
      <section className="overflow-hidden rounded-2xl border border-zinc-800 bg-zinc-900/60 shadow-2xl shadow-black/20">
        <div className="border-b border-zinc-800 px-4 py-4 lg:px-5"><div className="flex items-center gap-2"><Sparkles className="h-4 w-4 text-emerald-400"/><h2 className="font-semibold">Minerador V1</h2><span className="rounded bg-emerald-400/10 px-2 py-0.5 text-[10px] font-bold text-emerald-300">FASE 2</span></div><p className="mt-1 text-xs text-zinc-500">Encontre páginas conectadas a WhatsApp, checkout, VSL e tracking. Depois valide o domínio na Meta Ads Library.</p></div>
        <div className="grid gap-3 p-4 lg:grid-cols-[1.2fr_.65fr_.65fr_1fr_auto] lg:p-5">
          ...
          <div><label className="mb-1.5 block text-[11px] font-semibold text-zinc-500">PAÍS</label><div className="rounded-xl border border-zinc-700 bg-zinc-950 px-3 py-3 text-sm text-zinc-300">🇧🇷 Brasil</div></div>
