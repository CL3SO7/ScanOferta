...
import MetaConnectedMiner from './components/MetaConnectedMiner'
  ...
  return <div className="min-h-screen bg-[radial-gradient(circle_at_top_left,rgba(16,185,129,.08),transparent_28%),radial-gradient(circle_at_top_right,rgba(59,130,246,.07),transparent_28%)] bg-zinc-950 text-zinc-100">
    <header className="sticky top-0 z-30 border-b border-zinc-800/80 bg-zinc-950/90 backdrop-blur-xl">
      <div className="mx-auto flex max-w-[1700px] items-center justify-between gap-4 px-4 py-4 lg:px-8">
        <div className="flex items-center gap-3"><div className="grid h-10 w-10 place-items-center rounded-xl border border-emerald-400/20 bg-emerald-400/10 shadow-neon"><Radar className="h-5 w-5 text-emerald-400" /></div><div><h1 className="font-bold tracking-tight">Oferta Radar</h1><p className="text-[11px] text-zinc-500">minerador de ofertas & funis</p></div></div>
        <button onClick={() => setShowKey(true)} className="inline-flex items-center gap-2 rounded-xl border border-zinc-800 bg-zinc-900 px-3 py-2 text-xs font-semibold text-zinc-300 hover:border-zinc-700"><KeyRound className="h-4 w-4" /> {apiKey ? 'urlscan conectado' : 'Adicionar API Key'}{apiKey && <span className="h-2 w-2 rounded-full bg-emerald-400" />}</button>
      </div>
    </header>

    <main className="mx-auto max-w-[1700px] px-4 py-6 lg:px-8">
      <div className="mb-5 inline-flex rounded-xl border border-zinc-800 bg-zinc-900 p-1"><button onClick={()=>setEngine('keywords')} className={`rounded-lg px-4 py-2 text-xs font-bold ${engine==='keywords'?'bg-violet-500 text-white':'text-zinc-500'}`}>🔑 Oportunidades & Palavras</button><button onClick={()=>setEngine('inbox')} className={`rounded-lg px-4 py-2 text-xs font-bold ${engine==='inbox'?'bg-blue-500 text-white':'text-zinc-500'}`}>📥 Caixa de Entrada</button><button onClick={()=>setEngine('hunter')} className={`rounded-lg px-4 py-2 text-xs font-bold ${engine==='hunter'?'bg-blue-500 text-white':'text-zinc-500'}`}>🎯 Caçar Ofertas</button><button onClick={()=>setEngine('analyzer')} className={`rounded-lg px-4 py-2 text-xs font-bold ${engine==='analyzer'?'bg-fuchsia-500 text-white':'text-zinc-500'}`}>🟣 Analisar Oferta</button><button onClick={()=>setEngine('meta')} className={`rounded-lg px-4 py-2 text-xs font-bold ${engine==='meta'?'bg-blue-500 text-white':'text-zinc-500'}`}>🔵 Meta Hunt</button><button onClick={()=>setEngine('web')} className={`rounded-lg px-4 py-2 text-xs font-bold ${engine==='web'?'bg-emerald-400 text-zinc-950':'text-zinc-500'}`}>🟢 Web Radar</button><button onClick={()=>setEngine('library')} className={`rounded-lg px-4 py-2 text-xs font-bold ${engine==='library'?'bg-zinc-700 text-white':'text-zinc-500'}`}>⭐ Minhas Ofertas</button></div>
      {engine==='keywords' && <KeywordLab/>}
      {engine==='inbox' && <MetaInbox/>}
      {engine==='hunter' && <MetaConnectedMiner/>}
      {engine==='analyzer' && <OfferAnalyzer/>}
      {engine==='meta' && <MetaHunt candidates={preparedResults}/>} 
      ...
      <details className="mt-3 rounded-xl border border-zinc-800 bg-zinc-900/40"><summary className="cursor-pointer px-4 py-3 text-xs font-semibold text-zinc-500">Busca avançada / editar query do urlscan</summary><div className="flex flex-col gap-3 border-t border-zinc-800 p-4 lg:flex-row"><textarea value={query} onChange={e => setQuery(e.target.value)} rows={2} spellCheck={false} className="min-h-[52px] flex-1 resize-y rounded-xl border border-zinc-700 bg-zinc-950 px-4 py-3 font-mono text-xs text-zinc-300 outline-none focus:border-emerald-400/50"/><button onClick={() => runSearch()} className="rounded-xl border border-emerald-400/30 px-5 py-3 text-xs font-bold text-emerald-300">Executar query</button></div></details>
