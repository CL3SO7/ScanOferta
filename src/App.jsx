...
export default function App() {
  ...
  const [engine, setEngine] = useState('hunter')
  ...
  return <div className="min-h-screen bg-[radial-gradient(circle_at_top_left,rgba(16,185,129,.08),transparent_28%),radial-gradient(circle_at_top_right,rgba(59,130,246,.07),transparent_28%)] bg-zinc-950 text-zinc-100">
    ...
    <main className="mx-auto max-w-[1700px] px-4 py-6 lg:px-8">
      <div className="mb-5 inline-flex rounded-xl border border-zinc-800 bg-zinc-900 p-1"><button onClick={()=>setEngine('hunter')} className={`rounded-lg px-4 py-2 text-xs font-bold ${engine==='hunter'?'bg-blue-500 text-white':'text-zinc-500'}`}>🎯 Caçar Ofertas</button><button onClick={()=>setEngine('analyzer')} className={`rounded-lg px-4 py-2 text-xs font-bold ${engine==='analyzer'?'bg-fuchsia-500 text-white':'text-zinc-500'}`}>🟣 Analisar Oferta</button><button onClick={()=>setEngine('meta')} className={`rounded-lg px-4 py-2 text-xs font-bold ${engine==='meta'?'bg-blue-500 text-white':'text-zinc-500'}`}>🔵 Meta Hunt</button><button onClick={()=>setEngine('web')} className={`rounded-lg px-4 py-2 text-xs font-bold ${engine==='web'?'bg-emerald-400 text-zinc-950':'text-zinc-500'}`}>🟢 Web Radar</button><button onClick={()=>setEngine('library')} className={`rounded-lg px-4 py-2 text-xs font-bold ${engine==='library'?'bg-zinc-700 text-white':'text-zinc-500'}`}>⭐ Minhas Ofertas</button></div>
