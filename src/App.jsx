...
import PainSolutionMap from './components/PainSolutionMap'
  ...
  return <div className="min-h-screen bg-[radial-gradient(circle_at_top_left,rgba(16,185,129,.08),transparent_28%),radial-gradient(circle_at_top_right,rgba(59,130,246,.07),transparent_28%)] bg-zinc-950 text-zinc-100">
    ...
    <main className="mx-auto max-w-[1700px] px-4 py-6 lg:px-8">
      <div className="mb-6 flex flex-wrap gap-1 rounded-2xl border border-zinc-800/80 bg-zinc-900/70 p-1.5 shadow-xl shadow-black/10">
        <button onClick={()=>setEngine('keywords')} className={`rounded-xl px-4 py-2.5 text-xs font-semibold transition ${engine==='keywords'?'bg-zinc-100 text-zinc-950 shadow':'text-zinc-500 hover:bg-zinc-800 hover:text-zinc-200'}`}>Oportunidades</button><button onClick={()=>setEngine('pains')} className={`rounded-xl px-4 py-2.5 text-xs font-semibold transition ${engine==='pains'?'bg-rose-400 text-zinc-950 shadow':'text-zinc-500 hover:bg-zinc-800 hover:text-zinc-200'}`}>🧠 Dores & Soluções</button>
